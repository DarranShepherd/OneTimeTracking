...
class @TPClient.TargetProcess
  ...
  getStory: (storyId, ajax_opts = {}) ->
    story_url = @full_url + '/Userstories/' + storyId + '?include=[id, Name, Project, Feature[Epic]]'
    ajax_opts = @build_ajax_options.call this, ajax_opts
  ...
  getTaskDetail : (taskId, ajax_opts = {}) ->
    taskDetail_url = @full_url + '/Tasks/' + taskId + '?skip=0&take=999&include=[id,Name, TimeRemain, Project, UserStory[Name,Feature[Epic]], EntityType]'
    ajax_opts = @build_ajax_options.call this, ajax_opts
    $.ajax(taskDetail_url, ajax_opts)

  getBugDetail : (bugId, ajax_opts = {}) ->
    bugDetail_url = @full_url + '/Bugs/' + bugId + '?skip=0&take=999&include=[id,Name, TimeRemain, Project, UserStory[Name,Feature[Epic]], EntityType]'
    ajax_opts = @build_ajax_options.call this, ajax_opts
